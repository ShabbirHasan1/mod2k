...
A `#![no_std]` Rust crate for fast arithmetic modulo `2^k` and `2^k - 1`.
...
This crate provides a uniform interface for fast implementations of multiple moduli, allowing you to tune the balance:

- Primes: `2^7 - 1`, `2^13 - 1`, `2^31 - 1`, `2^61 - 1`. Slowest (~10 insn per multiplication, ~5 insn per addition), but have excellent quality.
